  ...
  return (
    <Box sx={{ borderRadius: 2, overflow: 'hidden', background: `linear-gradient(165deg, ${c.paper}, ${c.paperDark})`, boxShadow: `0 2px 8px ${c.bg}30` }}>
      ...
      <div style={{ width: '100%', height: 130, background: photo || defaultBg, position: 'relative', overflow: 'hidden' }}>
        ...
        {!photo && (
          <Stack direction="row" spacing={0.5} sx={{ position: 'absolute', bottom: 10, right: 12, opacity: 0.25, alignItems: 'center' }}>
      ...
      <Box sx={{ p: '12px 14px 14px', position: 'relative' }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mt: 1 }}>
          ...
          <Stack direction="row" spacing={0.75} alignItems="center">
            {showVisibility && (
              <Typography variant="caption" sx={{ color: isPrivate ? c.inkLight + '80' : c.inkLight + 'a0' }}>
                {isPrivate ? '🔒 仅彼此可见' : '🌐 公开'}
              </Typography>
            )}

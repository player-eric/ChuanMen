...
export interface EventData {
  ...
  spots: number;
  total: number;
  people: (string | { name: string; avatar?: string })[];
  phase: EventPhase;
  invitedBy?: string;
  desc: string;
  houseRules?: string;
...
export type FeedItem =
  ...
  | { type: 'activity'; name: string; title: string; date: string; location: string; spots: number; total?: number; people: (string | { name: string; avatar?: string })[]; film?: string; filmPoster?: string; scene?: string; navTarget?: string; waitlistCount?: number; taskSummary?: { role: string; claimerName?: string }[] } & FeedInteraction

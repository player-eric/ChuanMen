        ...
        <Card>
          <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
            ...
            {upcoming.map((evt, i) => {
              ...
              const dateStr = evt.startsAt ? new Date(evt.startsAt).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit', weekday: 'short' }) : '';
              return (
                <Box key={evt.id}>
            ...
            {past.map((evt, i) => (
              <Box key={evt.id}>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '3fr 1.5fr 1fr 1fr' }, gap: 1, px: 2, py: 1.5, alignItems: 'center' }}>
                  ...
                  <Typography variant="body2" sx={{ display: { xs: 'none', md: 'block' } }}>{evt.startsAt ? new Date(evt.startsAt).toLocaleDateString('zh-CN') : ''}</Typography>
          ...
          {cancelled.map((evt) => (
            <Grid key={evt.id} size={{ xs: 12, md: 6 }}>
              <Card><CardContent>
                ...
                <Typography variant="body2" color="text.secondary">Host: {evt.host?.name ?? '—'} · {evt.startsAt ? new Date(evt.startsAt).toLocaleDateString('zh-CN') : ''}</Typography>
              </CardContent></Card>
            </Grid>
          ))}
